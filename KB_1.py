...
list_of_users = {}
my_chats = [123456789] # Пример ID чата для демонстрации
kurs_trig = 0
...
try:
    with open('data.json', 'r') as f:
        ...
        bot.send_message(123456789, "бот был перезагружен")
        bot.send_message(123456789, "резервная копия рейтинга подгружена")
...
def get_text_messages(message):
    ...
    if message.from_user.id == 123456789:
        if message.text[0 : 4] == '/add':
            ...
            bot.send_message(123456789, "added!")
            # write_my_chats()
    if (message.chat.id in my_chats):
        ...
        if message.text == "💪 Топ активности":
            ...
            for i in range(len(sorted_list)):# переюор для просвоения юзернеймом
        ...
        bot.send_message(123456789, "НСД к боту")
        user_id = message.from_user.id
        bot.send_message(123456789, f"{message.chat.id}, @{bot.get_chat_member(user_id, user_id).user.username}")
